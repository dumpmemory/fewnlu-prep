...
from utils import InputFeatures, InputExample, get_verbalization_ids, chunks, trim_input_ids, remove_final_punc, \
    lowercase_first
...
class TaskHelper(ABC):
    ...
    def __init__(self, wrapper_config, tokenizer, preprocessor, model):
...
class MultiRcTaskHelper(TaskHelper):
    """A custom data_utils helper for the MultiRC dataset."""
...
class CopaTaskHelper(TaskHelper):
    """A custom data_utils helper for the COPA dataset."""

    def train_step(self, batch, **kwargs) -> Optional[torch.Tensor]:
        if self.config.wrapper_type == 'cls':
            return
        assert self.config.wrapper_type == 'mlm', 'train_step() for COPA is only implemented for MLM models'

        inputs = self.model.generate_default_inputs(batch)
        ...
        prediction_scores = self.model(**inputs)[0].view(-1, vocab_size)
    ...
    def eval_step(self, batch: Dict[str, torch.Tensor], decoding_strategy: str = 'default', **kwargs):
        if self.config.wrapper_type == 'cls':
            return
        assert self.config.wrapper_type == 'mlm', 'eval_step() for COPA is only implemented for MLM models'
    ...
    def _get_choice_log_probability(self, batch, target_sequence, decoding_strategy: str = 'default'):
        ...
        input_ids = trim_input_ids(
            batch['input_ids'], num_masks=num_masks, pad_token_id=self.tokenizer.pad_token_id,
        ...
        while True:
            ...
            original_batch["block_flags"] = batch["block_flags"]
            original_batch["token_type_ids"] = batch["token_type_ids"]

            inputs = self.model.generate_default_inputs(original_batch)
    ...
    def add_special_input_features(self, input_example: InputExample, input_features: InputFeatures) -> None: #TODO
        if self.config.wrapper_type == 'cls':
            return
        mask_start = input_features.input_ids.index(self.tokenizer.mask_token_id)

        for (i,choice) in enumerate(['choice1', 'choice2']):
            choice_text = input_example.meta[choice]
            choice_token_ids = get_verbalization_ids(choice_text, self.tokenizer, force_single_token=False)
            ...
            input_features.meta[f'pure_{choice}_token_ids']=choice_token_ids
        
        len1=len(input_features.meta[f'pure_choice1_token_ids'])
        len2=len(input_features.meta[f'pure_choice2_token_ids'])
        if len1<len2:
            input_features.meta[f'input_ids2']=copy.deepcopy(input_features.input_ids)
            input_features.meta[f'input_ids1']=copy.deepcopy(input_features.input_ids)
            input_features.meta[f'input_ids1'][mask_start+len1:-(len2-len1)]=input_features.input_ids[mask_start+len2:]
        elif len1>len2:
            input_features.meta[f'input_ids1']=copy.deepcopy(input_features.input_ids)
            input_features.meta[f'input_ids2']=copy.deepcopy(input_features.input_ids)
            input_features.meta[f'input_ids2'][mask_start+len2:-(len1-len2)]=input_features.input_ids[mask_start+len1:]
        else:
            input_features.meta[f'input_ids1']=copy.deepcopy(input_features.input_ids)
            input_features.meta[f'input_ids2']=copy.deepcopy(input_features.input_ids)
    ...
    def add_features_to_dict(self, features: List[InputFeatures], feature_dict: Dict[str, torch.Tensor]) -> None:
        ...
        for choice in ['choice1', 'choice2']:
            ...
            pure_choice_token_ids=[copy.deepcopy(f.meta[f'pure_{choice}_token_ids']) for f in features]
            feature_dict[f'pure_{choice}_token_ids']=torch.zeros((len(pure_choice_token_ids),np.max([len(x) for x in pure_choice_token_ids])),dtype=torch.long)
            for (i,padded_target) in enumerate(pure_choice_token_ids):
                feature_dict[f'pure_{choice}_token_ids'][i,:len(padded_target)]=torch.tensor(padded_target,dtype=torch.long)
        feature_dict[f'input_ids1']=torch.tensor([f.meta[f'input_ids1'] for f in features], dtype=torch.long)
        feature_dict[f'input_ids2']=torch.tensor([f.meta[f'input_ids2'] for f in features], dtype=torch.long)

class WscTaskHelper(TaskHelper):
    """A custom data_utils helper for the Wsc dataset."""
    id_to_target = []
    ...
    def add_features_to_dict(self, features: List[InputFeatures], feature_dict: Dict[str, torch.Tensor]) -> None:
        ...
        padded_targets=[f.meta['padded_target'] for f in features]
        feature_dict['padded_target']=torch.zeros((len(padded_targets),np.max([len(x) for x in padded_targets])),dtype=torch.long)
        for (i,padded_target) in enumerate(padded_targets):
            feature_dict['padded_target'][i,:len(padded_target)]=torch.tensor(padded_target,dtype=torch.long)

    def train_step(self, batch, **kwargs) -> Optional[torch.Tensor]:
        if self.config.wrapper_type == 'cls':
            return

        assert self.config.wrapper_type == 'mlm', 'train_step() for WSC is only implemented for MLM models'
    ...
    def eval_step(self, batch: Dict[str, torch.Tensor], decoding_strategy: str = 'default', **kwargs):
        if self.config.wrapper_type in ['sequence_classifier', 'span_pair_classifier']:
            return
        assert self.config.wrapper_type == 'mlm', 'eval_step() for WSC is only implemented for MLM models'
        assert batch['input_ids'].shape[0] == 1, 'eval_step() for COPA is only implemented for batch_size=1'
        ...
        orig_mask_positions = [
            idx for idx, input_id in enumerate(input_ids[0]) if input_id == self.tokenizer.mask_token_id
        ]
        ...
        while True:
            ...
            if not mask_positions:  # there are no masks left to process, we are done
                ...
                output_actual = self.tokenizer.decode([
                    ...
                    if idx in orig_mask_positions and input_id not in self.tokenizer.all_special_ids
                ])
