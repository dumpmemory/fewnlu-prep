...
from methods.utils import InputFeatures, InputExample, get_verbalization_ids, chunks, trim_input_ids, remove_final_punc, \
    lowercase_first
...
class TaskHelper(ABC):
    ...
    def __init__(self, wrapper):
        """
        Create a new data_utils helper.
    ...
    def train_step(self, batch: Dict[str, torch.Tensor], **kwargs) -> Optional[torch.Tensor]:
        """
        Custom implementation of the train step for this data_utils.
    ...
    def eval_step(self, batch: Dict[str, torch.Tensor], **kwargs) -> Optional[torch.Tensor]:
        """
        Custom implementation of the eval step for this data_utils.
    ...
    def get_sequence_classifier_inputs(self, example: InputExample) -> Dict[str, Any]:
        """
        Get the inputs for sequence classification. Override this method if the input for the data_utils considered is of a
...
class MultiMaskTaskHelper(TaskHelper):
    """A custom data_utils helper for classification datasets where multiple masks are required for one or more verbalizers."""
...
class WicTaskHelper(TaskHelper):
    """A custom data_utils helper for the WiC dataset."""
    ...
    def get_sequence_classifier_inputs(self, example: InputExample) -> Dict[str, Any]:
        ...
        return self.wrapper.tokenizer.encode_plus(text_a, example.text_b, add_special_tokens=True,
                                                  max_length=self.wrapper.config.max_seq_length, truncation=True)
...
class CopaTaskHelper(TaskHelper):
    ...
    def train_step(self, batch, **kwargs) -> Optional[torch.Tensor]:
        ...
        if self.wrapper.config.use_continuous_prompt:
            vocab_size = self.wrapper.model.model.config.vocab_size
        else:
            vocab_size = self.wrapper.model.config.vocab_size

        prediction_scores = self.wrapper.model(**inputs)[0].view(-1, vocab_size)
...
class RecordTaskHelper(TaskHelper):
    """A custom data_utils helper for the ReCoRD dataset."""
